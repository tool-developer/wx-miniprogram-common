...
class FS {
  ...
  constructor(options) {
    ...
    this.completed = false; //将fs对象暴露出去

    this.fs = fs; //服务器端资源路径

    this.serverURL = options.serverURL; //public文件处理-启动时先做服务器资源映射，防止文件下载失败

    this.mapPublicServerFiles(options.publics); //static文件处理

    this.mapStaticFiles(); //temp文件map映射

    this.mapTempUrls = {}; //创建目录

    let publicDirPath = self.getPublicFullPath();
    let staticDirPath = self.getStaticFullPath();
    self.toMkDir(publicDirPath);
    self.toMkDir(staticDirPath); //下载zip包数据
    ...
    if (options.zip) {
      ...
      self.mapPublicFiles(options.publics); //记作完成标记

      self.completed = true; //异步检测是否需要更新文件
  ...
  toDownloadZipFile(zipFile, callback) {
    ...
    self.downloadZip(zipFile, function (err) {
      ...
      fs.writeFileSync(signFileName, JSON.stringify(data), 'utf8'); //记作完成标记
  ...
  mapPublicServerFiles(files) {
    files = files || []; //未配置服务器端资源路径，不做服务器资源映射
  ...
  tsave(fileName, tempFilePath, callback) {
    ...
    return toReturn(new Promise(function (resolve, reject) {
      if (!fileName) {
        let err = 'save no file name'; //callback && callback(err);

        return reject(err);
      }

      if (!tempFilePath) {
        let err = 'save no temp file path'; //callback && callback(err);
    ...
    return toReturn(new Promise(function (resolve, reject) {
      ...
      fs.saveFile({
        ...
        success(res) {
          ...
          let err = 'save to static error'; //callback && callback(err);
  ...
  copy(fileName, tempFilePath, callback) {
    ...
    return toReturn(new Promise(function (resolve, reject) {
      if (!fileName) {
        let err = 'copy no file name'; //callback && callback(err);

        return reject(err);
      }

      if (!tempFilePath) {
        let err = 'copy no temp file path'; //callback && callback(err);
  ...
  get(fileName, type, callback) {
    ...
    return toReturn(new Promise(function (resolve, reject) {
        ...
        if (fullFileName) {
          ...
          return wx.getFileInfo({
            ...
            fail: function getFileInfoFail() {
              //
              let fileNameUrl = self.mapTempUrls[fileName]; //返回的是服务器地址
              ...
              let err = 'no this temp file'; //callback && callback(err);
              //
      ...
      if (type === STATIC_SIGN) {
        ...
        let err = 'no this static file'; //callback && callback(err);
        //
  ...
  download(options, fileName, callback) {
    ...
    let type = options.type; //处理filePath
    ...
    return toReturn(new Promise(function (resolve, reject) {
      ...
      options['success'] = function (res) {
        //
        let filePath = res.filePath || res.tempFilePath; //注入到temps

        if (type === TEMP_SIGN) {
          //
          self.temps[fileName] = res.tempFilePath; //映射服务器地址
...
export { FS, FS_SIGN, FS as default };
